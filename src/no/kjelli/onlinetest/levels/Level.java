package no.kjelli.onlinetest.levels;
...
import no.kjelli.onlinetest.gameobjects.Block;
import no.kjelli.onlinetest.gameobjects.Bomb;
import no.kjelli.onlinetest.gameobjects.Destructible;
import no.kjelli.onlinetest.gameobjects.Floor;
import no.kjelli.onlinetest.gameobjects.Player;
import no.kjelli.onlinetest.gameobjects.PlayerMP;
import no.kjelli.onlinetest.network.Network;
import no.kjelli.onlinetest.network.Packet;
import no.kjelli.onlinetest.network.PacketLevelRequest;
import no.kjelli.onlinetest.network.PacketLevelResponse;
import no.kjelli.onlinetest.network.PacketPlayerAdd;
import no.kjelli.onlinetest.network.PacketPlayerPlaceBomb;
import no.kjelli.onlinetest.network.PacketPlayerRemove;
import no.kjelli.onlinetest.network.PacketPlayerUpdate;
...
public class Level {
	...
	public static void init(String filename) {
		...
		World.pause(OnlineTest.tag_playfield, true);
	}
	...
	public static void start() {
		...
		World.hide(OnlineTest.tag_playfield, false);
		World.pause(OnlineTest.tag_playfield, false);
	}
	...
	private static void loadFromMap(char[][] map) {
		setWidth(map.length * OnlineTest.block_size);
		setHeight(map[0].length * OnlineTest.block_size);
	...
	private static void loadFromFile(String name) {
		...
		try {
			...
			setWidth((int) (width * OnlineTest.block_size));
			setHeight((int) (height * OnlineTest.block_size));
	...
	private static void placeBomb(PacketPlayerPlaceBomb packet) {
		...
		World.add(new Bomb(packet.x, packet.y, source, source.power));
	}
