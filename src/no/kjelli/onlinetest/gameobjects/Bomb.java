...
import no.kjelli.onlinetest.gameobjects.Fire;
...
public class Bomb extends AbstractCollidable {
	...
	public Bomb(int x_index, int y_index, Player source, int power) {
		this(x_index, y_index, source, power, true);
	}

	public Bomb(int x_index, int y_index, Player source, int power,
			boolean ticking) {
		super(
				x_index * OnlineTest.block_size + OnlineTest.block_size / 2 - 6,
				y_index * OnlineTest.block_size + OnlineTest.block_size / 2 - 6,
				12, 12);
